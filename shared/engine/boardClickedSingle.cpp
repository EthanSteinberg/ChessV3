...
std::vector<t_message> t_chessEngine::boardClickedSingle(const t_message &message)
{
   ...
   else
   {
      ...
      if (selected)
      {
         ...
         else if (std::find(pawnMove.begin(), pawnMove.end(), pos) != pawnMove.end())
         {
            moveMove(pos,buffer);

            enPessantPossible = 2;
            enPessantPos.x = pos.x;
            enPessantPos.y = pos.y - 1 + 2 * turn;

            enPessantPawn = pos;

            std::cout<<"The pos is "<<enPessantPos<<" and "<<enPessantPawn<<std::endl;
         }

         else if (std::find(pawnPessant.begin(), pawnPessant.end(), pos) != pawnPessant.end())
         {
            pessantMove(pos,buffer);
         }
